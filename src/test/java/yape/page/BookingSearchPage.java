...
import yape.PageObjects.BookingSearchPageObjects;

import java.util.Collections;
import java.util.List;

import static yape.PageObjects.BookingSearchPageObjects.*;
...
public class BookingSearchPage extends WebDriverHelper{
    ...
    public void setDestination(String destination){
        ...
        List<WebElement> locationResultList = getElementList(bookingSearchPageObjects.DESTINATION_LIST_LOC);

        if(!locationResultList.isEmpty()){
            for(WebElement elem :  locationResultList){
                String cityDestination = elem.findElement(bookingSearchPageObjects.DESTINATION_CITY_TITLE_LOC).getText();
                String areaDestination = elem.findElement(bookingSearchPageObjects.DESTINATION_CITY_SUB_LOC).getText();
    ...
    public void skipSignInScreen(){
        ...
        boolean isVisible = waitVisibility(genericByBuilder("Enter your destination"), 5)
    ...
    private void setInputValues(int index, String key, String value){
        ...
        //GET TEXT VALUE
        String ElemTextVal = "";
        ElemTextVal = StringUtils.isNotEmpty(valueElem.getAttribute("text")) ? valueElem.getAttribute("text") : "";
        ...
        if(areElementsPresent){
            ...
            if(StringUtils.isNotEmpty(ElemTextVal) && Integer.parseInt(ElemTextVal) > Integer.parseInt(value)){
            ...
            }else if(StringUtils.isNotEmpty(ElemTextVal) && Integer.parseInt(ElemTextVal) < Integer.parseInt(value)){
                    ...
                    .forEach(
                            value -> {
                                ...
                                try {
                                    ...
                                    throw new SkipException(String.format("Path specified on table doesn't exist: %s", AGE));
                                }
    ...
    public void selectFromResultList(int index){
        ...
        if(!locationResultList.isEmpty()){
            WebElement item = locationResultList.size() >= index && locationResultList.get(index) != null ? locationResultList.get(index) : locationResultList.get(0);
            if(item != null){
                item.click();
            }else{
